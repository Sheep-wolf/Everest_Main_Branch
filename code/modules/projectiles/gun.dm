...
/obj/item/weapon/gun/proc/Fire(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, params, reflex = 0)//TODO: go over this
	...
	if(ishuman(user))
		if(user.dna && user.dna.mutantrace == "adamantine")
			...
			return
	if(double_action == 1 && hammer_back != 1)// needs to have some reset on the hammer. revisit this eventually, not now though - Sheep. Revisiting, also, if we have a [(whatever we call the gun icon var)] we can make the icons work too!.
		hammer_back = 1
		var/mob/living/M = user
		M << "\blue You click the hammer back on the gun!"
		icon_state = "[gun_skin]"
		return
	...
	if(silenced)
		...
		if(double_action == 1 && hammer_back == 1)
			hammer_back = 0
			icon_state = "[gun_skin]1"
