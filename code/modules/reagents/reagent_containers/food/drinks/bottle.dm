...
/obj/item/weapon/reagent_containers/food/drinks/bottle/attack(mob/living/target as mob, mob/living/user as mob)
	...
	msg_admin_attack("[key_name_admin(user)] attacked [key_name_admin(target)] with a bottle. (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
...
/obj/item/weapon/reagent_containers/food/drinks/bottle/eggnog
	name = "Eggnog"
	desc = "How else can you get sozzled on christmass and get away with it?"
