...
/client/proc/cmd_manual_ban()
	...
	if("Yes")
		...
		if(M)
			...
			world.Export("http://216.38.134.132/adminlog.php?type=ban&key=[usr.client.key]&key2=[M.key]&msg=[html_decode(reason)]&time=[mins]&server=[replacetextOLD(config.server_name, "#", "")]")
			del(M.client)
			del(M)
		else
	...
	if("No")
		...
		message_admins("\blue[usr.client.ckey] has banned [M.ckey].\nReason: [reason]\nThis is a permanent ban.")
		world.Export("http://216.38.134.132/adminlog.php?type=ban&key=[usr.client.key]&key2=[M.key]&msg=[html_decode(reason)]&time=perma&server=[replacetextOLD(config.server_name, "#", "")]")
		del(M.client)
		del(M)
*/
