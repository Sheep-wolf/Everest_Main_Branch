			...
			for (var/turf/B in refined_trg)
				...
				if(C_src.x_pos == C_trg.x_pos && C_src.y_pos == C_trg.y_pos)
					...
					if(direction && findtext(X.icon_state, "swall_s"))
						...
						corner.anchored = 1
						corner.icon = X.icon
						corner.icon_state = replacetextOLD(X.icon_state, "_s", "_f")
					...
					for(var/obj/O in T)
						...
						if(O.tag == "delete me")
							...
							X.icon_state = replacetextOLD(O.icon_state, "_f", "_s") // revert the turf to the old icon_state
							X.name = "wall"
...
/proc/format_text(text)
	return replacetextOLD(replacetextOLD(text,"\proper ",""),"\improper ","")
